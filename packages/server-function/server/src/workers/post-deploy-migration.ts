...
// function getJobDataLoggingFields(job: any): Record<string, string> {
//   return {
...
export const initPostDeployMigrationWorker = (config: any) => {
  ...
  return { queue: null, name: PostDeployMigrationQueueName };
};
...
export async function addPostDeployMigrationJobData<T extends PostDeployJobData>(
  ...
  // Runs synchronously instead of queuing
  const repo = getSystemRepo();
  if (jobData.type === 'dynamic') {
    await runDynamicMigration(repo, undefined, jobData as unknown as DynamicPostDeployJobData);
  } else if (jobData.type === 'custom') {
    // Custom migrations should be called directly with runCustomMigration
    // since the callback is not available here
    globalLogger.warn('Custom post-deploy migration should be executed directly with runCustomMigration');
  }
  return undefined;
}
...
async function runDynamicMigration(
  repo: Repository,
  job: any | undefined,
  jobData: DynamicPostDeployJobData
): Promise<PostDeployJobRunResult> {
  const asyncJob = await repo.readResource<AsyncJob>('AsyncJob', jobData.asyncJobId);
  const exec = new AsyncJobExecutor(repo, asyncJob);
  try {
    const results = await withLongRunningDatabaseClient(async (client) => {
      return executeMigrationActions(client, jobData.migrationActions);
    });
    const output = getAsyncJobOutputFromMigrationActionResults(results);
    await exec.completeJob(repo, output);
  } catch (err: any) {
